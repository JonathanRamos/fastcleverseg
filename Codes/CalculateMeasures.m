...
function measures = CalculateMeasures(Seg, GT)
       Seg = Seg ==1;
       GT = GT ==1;
      
       Ared = imresize3(uint8(Seg), 0.5) > 0;
       Bred = imresize3(uint8(GT), 0.5) > 0; 
       [r,c,v] = ind2sub(size(Ared), find(bwperim(Ared, 26) > 0));  
       AA = double([r,c,v]);
       [r,c,v] = ind2sub(size(Bred), find(bwperim(Bred, 26) > 0));
       BB = double([r,c,v]);  

       valores = pdist2(AA, BB);
       
       minab = mean(min(valores, [], 1));
       minba =  mean(min(valores, [], 2));
        
       measures(3) = 2* max([minab minba]); % computeHD(AA, BB, method); % Original Hausdorff distance
       C = confusionmat(GT(:), Seg(:));
       TP = C(1,1);
       FP = C(2,1);
       TN = C(2,2);
       FN = C(1,2);
       FPR = FP/(FP+TN);
       FNR = FN/(FN+TP);
       measures(1) = ((TP*TN)-(FP*FN))/(sqrt((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))); % Matthews correlation coefficient
       measures(2) = 1 - ((FPR+FNR)/2); % AUC
       measures(4) = (TP+TN)/(TP+TN+FP+FN); %% Accuracy
       measures(5) = TP/(TP+FP); % Precision
       measures(6)= TP/(TP+FN); % Sensitivity or recall
       measures(7) = TN/(TN+FP); % Specificity
       measures(8) = (2*measures(5)*measures(6))/(measures(5)+measures(6));%(2*(medidas(5)*medidas(6)))/(medidas(5)+medidas(6)); % F-measure
       [hd D] = HausdorffDist(AA,BB);
       if (isempty(hd))
          hd=0; 
       end
       measures(9) = 2* hd; % Hausdorf
end
